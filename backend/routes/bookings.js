...
router.get('/my', protect, async (req, res) => {
  try {
    const bookings = await Booking.find({ user: req.user._id })
      .populate('flight', 'flightNumber airline origin destination departureDate departureTime arrivalTime originCode destinationCode')
      .sort({ createdAt: -1 });
    res.json(bookings);
  } catch (err) {
    res.status(500).json({ message: err.message });
...
router.put('/:id/cancel', protect, async (req, res) => {
  try {
    ...
    if (flight) {
      const seat = flight.seats.find(s => s.seatNumber === booking.seatNumber);
